...
| `MAX_JOB_ATTEMPTS`                   | int  | 3                                                        | Maximum worker assignments for one scan, including the initial assignment. An expired final attempt is dead-lettered                                         |
| `QUEUE_METRICS_REFRESH_SECONDS`      | int  | 60                                                       | Interval between cached queue snapshots. Each refresh performs one aggregate database query; metric scrapes and API reads do not query the database          |
| `PERFORMANCE_METRICS_REFRESH_SECONDS` | int | 15 \* 60                                                 | Idle interval between compact performance projection and publication passes                                                                                 |
| `PERFORMANCE_PROJECTION_BATCH_SIZE`  | int  | 100                                                      | Maximum terminal outcomes and reports projected in one database transaction                                                                                 |
| `PERFORMANCE_PROJECTION_BACKFILL_PAUSE_SECONDS` | int | 1                                             | Pause between full projection batches while historical work remains                                                                                         |
|                                      |      |                                                          |                                                                                                                                                             |
