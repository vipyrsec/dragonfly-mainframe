...
def test_report(
    ...
    report = ObservationReport(
        ...
        extra={"yara_rules": ["rule 1", "rule 2"]},
    )
    scan = Scan(
        name="c",
        version="1.0.0",
        ...
        queued_by="remmy",
        pending_at=datetime.now(UTC) - timedelta(seconds=30),
        pending_by="remmy",
...
def test_report_package_not_on_pypi():
    ...
    mock_httpx_client.configure_mock(**{"get.return_value.status_code": status.HTTP_404_NOT_FOUND})
...
def test_report_inspector_url(body_url: str | None, scan_url: str | None):
    assert _validate_inspector_url("a", "1.0.0", body_url, scan_url) == "test url"


@pytest.mark.parametrize(
    ("scans", "name", "version", "expected_status_code"),
    [
        ([], "a", "1.0.0", status.HTTP_404_NOT_FOUND),
        (
            [
                Scan(
                    name="c",
